...
class ControllerModulelpbc extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			$this->model_setting_setting->editSetting('lpbc', $this->request->post);
        ...
        if (isset($this->error['image'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/lpbc', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

		$this->data['action'] = $this->url->link('module/lpbc', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['modules'] = array();

		if (isset($this->request->post['lpbc_module'])) {
			$this->data['modules'] = $this->request->post['lpbc_module'];
		} elseif ($this->config->get('lpbc_module')) {
			$this->data['modules'] = $this->config->get('lpbc_module');
		}
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/lpbc')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		if (isset($this->request->post['lpbc_module'])) {
			foreach ($this->request->post['lpbc_module'] as $key => $value) {
